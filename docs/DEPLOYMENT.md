...
- **Who is an admin:** any advisor whose `advisors/{username}` doc has `isAdmin: true`. Toggle it on the portal's **Advisors** tab — `firestore.rules` reads the flag live, so promoting or demoting someone needs **no code change and no rules deploy**. Admins differ from plain advisors in exactly one way: they can add and remove people. All content permissions are equal for every advisor. The only hardcoded entry is the break-glass owner (`isOwnerAdmin` in `firestore.rules`, `OWNER_ADMIN_EMAILS` in `src/admin-roles.js`, and the allowlist in `storage.rules` — the build fails if the three drift). Changing *that* needs a rules deploy, and note the Advisors-tab Remove button does **not** revoke it. See `docs/SUCCESSION_CHECKLIST.md` §3.
