...
- **check-resource-links.mjs** 🔑 — pulls every resource's `url` and `actionLinks` from Firestore (one read) and makes a real HTTP request against each to flag anything dead, redirected to a new domain, or needing a manual browser check because the site blocks automation, redirects to a bot challenge, rate-limits, times out, has a certificate chain Node won't verify, or returns a temporary server error. It also flags a **deep link that now redirects to the site homepage** (`warn-path-collapsed`) — that returns 200 on the same host, so it would otherwise count as perfectly healthy while no longer taking a student anywhere useful. HEAD gets a short 5s probe and any HEAD failure is re-checked with GET before a link can be called broken, because several resource sites reject or stall HEAD while serving GET fine. Runs weekly via `.github/workflows/check-resource-links.yml`, which opens/updates a single tracking GitHub Issue labeled `link-check` and uploads both JSON reports as a 90-day workflow artifact. The issue is built for a weekly skim: severity in the title, breaks first, everything else collapsed, and **403s/bot-challenges grouped by host into one counted line** rather than listed individually — from GitHub's Azure IPs, mass.gov and a dozen similar sites refuse automation outright, which was ~24 of ~28 weekly findings and buried the handful that mattered. A comment (and therefore an email) is posted **only when the finding set actually changes** — something new, something cleared, or something recategorized — diffed against the base64 state blob carried in the issue body, which also drives the "seen N weeks in a row" counter. A quiet week refreshes the body silently. This only checks liveness — it does not judge whether a *better*, more specific page exists (that needs a human/AI reading the page; ask for a manual audit pass for that). Usage: `node scripts/check-resource-links.mjs [--json=report.json] [--concurrency=5]`. Unit-tested in `tests/unit/resource-link-check.test.mjs` (classifiers plus the request layer, against a throwaway local server).
- **check-resource-content-risk.mjs** 🔑 — pulls live resources from Firestore and builds the re-verification queue. **Only the clock puts a card in the queue**: `lastVerified` missing, malformed, or older than its category's recheck window (3 months for food/housing/health/legal-aid/immigration, 4 for money/jobs/family/consulates, 6 for the rest). Content signals — exact hours, a dollar amount, seasonal wording, intake rules, eligibility rules — do **not** create entries; they only say *what to look at* on a card that is already due, and break ties in the ordering.

  That split was a Sep 2026 rewrite. The previous version emitted a finding per risk signal and flagged **82 of 89 resources every week**: 46 hits for being in a high-risk category (a permanent attribute already encoded in the 3-month window, so it was double-counted) plus 71 for mentioning hours/prices/eligibility/intake — signals that could only be cleared by deleting the very information students need. 61 of the 82 were unresolvable by any action, and the other 21 needed a `lastVerified` field that had no control anywhere in the portal. The rewrite plus the new **Verified today** button (see below) took the queue to 21 genuinely-never-verified cards, each clearable in one click.

  Runs monthly via `.github/workflows/check-resource-content.yml` (1st of the month; it used to ride along with the weekly link check, which was ~13x oversampling for a 3-month window). Maintains one tracking Issue labeled `resource-review`: newly-due items listed individually, older ones collapsed, a comment only when the queue actually changes, and the issue closes when the queue empties. Usage: `node scripts/check-resource-content-risk.mjs [--json=report.json]`. Unit-tested in `tests/unit/resource-content-review.test.mjs`.

  **Clearing an entry:** press **Verified today** on the resource card in the advisor portal's Manage tab. That stamps `lastVerified` to the current `YYYY-MM` via a partial `updateDoc` (`markResourceVerified` in `src/admin-manage.js`), which works because Firestore validates the *merged* document on an update — pinned by tests in `tests/rules/firestore-rules.test.mjs`. Before that button existed the field was script-only and the queue could never be emptied.
