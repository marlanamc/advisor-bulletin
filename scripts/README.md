...
One-off and recurring maintenance tools. Run them from the repository root with Node 22 (`nvm use`).
...
**Archive convention:** dated scripts in `scripts/archive/` are single-use production fixes,
imports, or copy edits that already ran. Keep reusable, recurring, npm-script, and workflow
...
- **check-resource-links.mjs** 🔑 — pulls every resource's `url` and `actionLinks` from Firestore (one read) and makes a real HTTP request against each to flag anything dead, redirected to a new domain, or needing a manual browser check because the site blocks automation, redirects to a bot challenge, rate-limits, times out, or returns a temporary server error. Runs weekly via `.github/workflows/check-resource-links.yml`, which opens/updates a single tracking GitHub Issue labeled `link-check` when it finds links to review, tracks how many weeks each finding has repeated, and closes it once everything's clean again. This only checks liveness — it does not judge whether a *better*, more specific page exists (that needs a human/AI reading the page; ask for a manual audit pass for that). Usage: `node scripts/check-resource-links.mjs [--json=report.json] [--concurrency=5]`.
