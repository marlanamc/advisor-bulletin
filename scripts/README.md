...
- **check-resource-content-risk.mjs** 🔑 — pulls live resources from Firestore and builds an advisor review queue for cards likely to go stale: old/missing `lastVerified`, high-risk categories, exact hours, specific prices, seasonal language, intake/registration rules, or eligibility language. Runs weekly in `.github/workflows/check-resource-links.yml` and updates a single tracking GitHub Issue labeled `resource-review`. This does not decide whether a card is wrong; it prioritizes human re-verification. Usage: `node scripts/check-resource-content-risk.mjs [--json=report.json]`.
